...
import { CardsThreeIcon, LockKeyOpenIcon, PlayIcon, SparkleIcon, StarFourIcon, XIcon } from "./icons";
...
export function WorldCardAlbum({ catalog, rewards }: { catalog: PlatformCatalog; rewards: RewardState }) {
  ...
  return <section className="reward-panel world-album">
    ...
    <div className="world-album__sets">
      {catalog.worlds.map((world) => <section key={world.id} className={`card-set card-set--${world.id}`}><header><span style={{ background: world.accent }}/><div><small>Set de mundo</small><h3>{world.label}</h3></div><strong>{catalog.cards.filter((card) => card.worldId === world.id && rewards.unlockedCardIds.includes(card.id)).length}/4</strong></header><div>{catalog.cards.filter((card) => card.worldId === world.id).map((card, index) => {
