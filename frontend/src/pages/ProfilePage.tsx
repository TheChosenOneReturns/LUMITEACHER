...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([api.getRewards(), api.getCatalog()])
      ...
      .catch(() => setError("¡Ups! No pudimos cargar tus datos. Probemos de nuevo."));
  }, [isStudent]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (displayName.length < 2) { setError("El nombre tiene que tener al menos 2 letras."); return; }
    try {
      if (isStudent && draft.selectedAccessoryId !== profile!.selectedAccessoryId) {
        const nextRewards = await api.selectAccessory(draft.selectedAccessoryId ?? null);
    ...
    } catch { setError("¡Ups! No pudimos guardar. Probemos de nuevo."); }
  }

  function selectOutfit(outfitId: string | null) {
    ...
    <div className="profile-page profile-studio page-width page-section">
      <motion.section className="profile-studio-hero" initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="profile-studio-hero__copy">
          <span className="eyebrow">{isStudent ? profile!.profileBadge ?? "Mi personaje" : profile!.adultLabel ?? "Perfil adulto"}</span>
          <h1>{isStudent ? `¡Hola, ${profile!.displayName}!` : `Hola, ${profile!.displayName}`}</h1>
          <p>{isStudent ? "Cambiá tu personaje, probá la ropa que ganaste y elegí tu mundo favorito." : "Configurá cómo te ven los alumnos y administrá tu acceso local."}</p>
        </div>
      ...
      <div className="profile-studio-grid">
        <motion.form className="profile-editor" onSubmit={submit} initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }}>
          <div className="studio-section-heading"><span>01</span><div><h2>{isStudent ? "Elegí tu personaje" : "Datos del perfil"}</h2></div></div>
          ...
          {isStudent ? <fieldset className="profile-character-picker"><legend>Elegí un animal o una persona</legend><div>{filteredAvatars.map((avatar) => {
            const unlocked = avatar.base || Boolean(rewards?.unlockedAvatarIds.includes(avatar.id));
          ...
          <button className="button button--primary profile-save" type="submit">{saved ? <CheckCircleIcon weight="fill" /> : <FloppyDiskIcon weight="bold" />}{saved ? "¡Listo!" : "Guardar"}</button>
        </motion.form>

        <motion.aside className="profile-wardrobe" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }}>
          {isStudent ? <>
            <div className="studio-section-heading"><span>02</span><div><h2>Mi vestidor</h2></div></div>
