...
export function ReadingPage() {
  ...
  if (!story || !adventure || !scene) return <LoadingState message="Abriendo tu cuento…" />;
  ...
  if (story.input.storyMode !== "interactive") {
    return (
      <div className="page-width page-section reading-page reading-page--classic">
        ...
        {pageIndex === readingPages.length - 1 ? <section className="reading-next"><Lumi message={language === "en" ? "Did you find every clue?" : "¿Encontraste todas las pistas?"} /><Link className="button button--green" to={`/historias/${story.storyId}/desafio`}>{language === "en" ? "Start challenge" : "¡Ir a las preguntas!"} <ArrowRightIcon /></Link></section> : null}
