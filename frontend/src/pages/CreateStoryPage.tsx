...
const difficultyOptions: { value: Difficulty; label: string; hint: string }[] = [
  { value: "facil", label: "Explorador", hint: "Pistas claras y fáciles" },
  { value: "media", label: "Aventurero", hint: "Un poco más difícil" },
  { value: "desafio", label: "Gran detective", hint: "Para quienes leen mucho" },
];
const stepLabels = ["Tipo", "Mundo", "Personaje", "Desafío"];
const generationPhases = ["Creando el mundo", "Inventando los caminos", "Dando vida a las voces", "Pensando las preguntas"];
    ...
    return (
      <section className="generation-state generation-state--cinematic page-width" aria-live="polite">
        ...
        <div className="generation-state__content">
          ...
          <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>¡Tu aventura está naciendo!</motion.h1>
    ...
    <div className="page-width page-section create-page create-page--studio">
      <motion.header className="story-studio-hero" initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="story-studio-hero__copy">
          <h1>Vos decidís qué pasa en la historia</h1>
          <p>Elegí un mundo, inventá un personaje y escuchá la aventura con voz suave.</p>
      ...
      {/* <section className="studio-presets" aria-labelledby="studio-presets-title">
        <div><span className="eyebrow"><SparkleIcon weight="fill" /> Aventuras listas</span><h2 id="studio-presets-title">Cuentos armados</h2></div>
        ...
        <div className="story-studio__workspace">
          ...
          <AnimatePresence initial={false}>
            <motion.section key={step} className="studio-step" initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -28 }} transition={{ type: "spring", stiffness: 190, damping: 24 }}>
              {step === 0 ? <>
                <span className="eyebrow"><PathIcon /> Tipo de cuento</span><h2>¿Cómo querés leer?</h2>
                <div className="experience-choice-grid">
                  {([{ value: "interactive", title: "Aventura con caminos", text: "Vos elegís qué pasa. Tiene cuatro finales distintos.", Icon: TreeStructureIcon }, { value: "classic", title: "Cuento seguido", text: "Leés de principio a fin, con dibujos.", Icon: PencilSimpleIcon }] as const).map(({ value, title, text, Icon }) => <motion.button key={value} type="button" className={storyMode === value ? "is-selected" : ""} aria-pressed={storyMode === value} onClick={() => setStoryMode(value)} whileHover={{ y: -5 }} whileTap={{ scale: .98 }}><Icon size={35} weight="duotone" /><span><strong>{title}</strong><small>{text}</small></span><AnimatePresence>{storyMode === value ? <SelectionMark /> : null}</AnimatePresence></motion.button>)}
                </div>
                <div className="language-switch"><span><TranslateIcon size={25} /><b>Idioma del cuento</b></span><div><button type="button" aria-pressed={language === "es"} className={language === "es" ? "is-selected" : ""} onClick={() => setLanguage("es")}><strong>ES</strong><span>Español</span></button><button type="button" aria-pressed={language === "en"} className={language === "en" ? "is-selected" : ""} onClick={() => setLanguage("en")}><strong>EN</strong><span>English</span></button></div></div>
              </> : null}

              {step === 1 ? <>
                <span className="eyebrow"><SparkleIcon /> El escenario</span><h2>¿Qué edad tenés y qué mundo te gusta?</h2>
              ...
              {step === 2 ? <>
                <span className="eyebrow"><PencilSimpleIcon /> Tu protagonista</span><h2>¿Quién es el héroe del cuento?</h2><p>Inventá un personaje y decile a Lumi qué querés aprender.</p>
                <label className="studio-character-field"><span>¿Quién protagoniza?</span><input value={mainCharacter} maxLength={60} placeholder="Ej.: una gata astronauta llamada Kira" onChange={(event) => setMainCharacter(event.target.value)} /><small>{mainCharacter.length}/60 · Si no ponés nada, Lumi inventa uno.</small></label>
                <label className="studio-objective-field"><span>¿Qué querés aprender?</span><strong>Escribí qué te gustaría practicar</strong><textarea aria-label="¿Qué te gustaría practicar o aprender?" value={educationalObjective} minLength={5} maxLength={160} rows={4} required onChange={(event) => setEducationalObjective(event.target.value)} /><small><ShieldCheckIcon /> No escribas tu nombre completo ni datos personales · {educationalObjective.length}/160</small></label>
              </> : null}

              {step === 3 ? <>
                <span className="eyebrow"><MagicWandIcon /> Último paso</span><h2>¿Qué tan difícil lo querés?</h2><p>Elegí la dificultad y cuánto querés leer.</p>
                <div className="studio-final-grid"><fieldset><legend>Dificultad</legend>{difficultyOptions.map((option) => { const Icon = difficultyIcons[option.value]; return <button type="button" key={option.value} className={difficulty === option.value ? "is-selected" : ""} aria-pressed={difficulty === option.value} onClick={() => setDifficulty(option.value)}><Icon size={26} weight="duotone" /><span><strong>{option.label}</strong><small>{option.hint}</small></span>{difficulty === option.value ? <SelectionMark /> : null}</button>; })}</fieldset><fieldset><legend>¿Cuánto querés leer?</legend>{([{ value: 300, label: "Cortito", hint: "8–12 minutos" }, { value: 800, label: "Mediano", hint: "15–22 minutos" }, { value: 1200, label: "Largo", hint: "25–35 minutos" }] as const).map((option) => <button type="button" key={option.value} className={maxWords === option.value ? "is-selected" : ""} aria-pressed={maxWords === option.value} onClick={() => setMaxWords(option.value)}><ClockIcon size={26} /><span><strong>{option.label}</strong><small>{option.hint}</small></span>{maxWords === option.value ? <SelectionMark /> : null}</button>)}</fieldset></div>
          ...
          <div className="studio-navigation"><button className="button button--outline" type="button" disabled={step === 0} onClick={() => setStep((value) => Math.max(0, value - 1))}>Atrás</button>{step < stepLabels.length - 1 ? <motion.button className="button button--yellow" type="button" onClick={() => setStep((value) => Math.min(stepLabels.length - 1, value + 1))} whileTap={{ scale: .97 }}>Siguiente <PathIcon /></motion.button> : <motion.button className="button button--yellow studio-create-button" type="submit" whileHover={{ y: -4 }} whileTap={{ y: 2, scale: .98 }}><MagicWandIcon size={24} /> ¡Crear mi {storyMode === "interactive" ? "aventura" : "cuento"}!</motion.button>}</div>
        </div>

        {/* <aside className="story-studio__preview" aria-label="Vista previa del cuento">
          <span className="preview-live"><i /> Así va quedando</span><div className="story-studio__preview-image"><motion.img key={sceneImage} src={sceneImage} alt="" initial={{ opacity: 0 }} animate={{ opacity: 1 }} /><motion.span animate={reduceMotion ? {} : { x: [0, 10, 0], y: [0, -4, 0] }} transition={{ duration: 4, repeat: Infinity }}><StoryThemeIcon theme={selectedTheme} size={48} /></motion.span></div>
          <h3>{loadedPreset ? storyPresets.find((preset) => preset.id === loadedPreset)?.title : language === "es" ? "Tu historia espera" : "A story waiting to be discovered"}</h3><p>{mainCharacter || (language === "es" ? "Lumi elige un personaje para vos." : "Lumi will choose a hero for you.")}</p>
          {step !== 2 ? <label className="preview-objective-editor"><span>¿Qué querés aprender?</span><textarea aria-label="¿Qué te gustaría practicar o aprender?" rows={3} value={educationalObjective} onChange={(event) => setEducationalObjective(event.target.value)} /></label> : null}
          <div className="preview-story-flow"><span className="is-ready"><CheckIcon /> Mundo</span><b /><span className={storyMode === "interactive" ? "is-ready" : ""}><TreeStructureIcon /> Decisiones</span><b /><span><SparkleIcon /> Final</span><b /><span><ShieldCheckIcon /> Preguntas</span></div>
          <div className="preview-voice"><SpeakerHighIcon weight="fill" /><span><strong>{language === "es" ? "Cuento en español" : "English narration"}</strong><small>Con voz suave para escuchar</small></span></div>
        </aside> */}
      </form>
    </div>
  );
